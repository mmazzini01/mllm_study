...
class MetricsAnalyzer:
    ...
    def perform_paired_t_tests(self):
        ...
        for metric_name, column_name in metrics.items():
            for chooser_key, chooser_col in chooser_cols:
                ...
                user_paired_df.to_csv(os.path.join(output_dir, f" {metric_name}_{chooser_col}.csv"), index=True)
    ...
    def save_summary_statistics(self, filename=None, results=None, words_results=None, 
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            if antonella_model_results is not None:
                ...
                f.write("Comparison: Model Preferred vs. Rejected Responses\n\n")
    ...
    def create_combined_boxplots_from_paired_data(self, output_dir=None):
        ...
        for chooser_key, chooser_col in chooser_cols:
            ...
            # Ensure x-axis labels are hidden on all subplots
            for ax in fig.axes:
                ax.set_xlabel("")
            ...
            fig.savefig(plot_filename, dpi=300, bbox_inches='tight')
            pdf_filename = plot_filename.replace('.png', '.pdf')
            for ax in fig.axes:
            ...
            for idx, (metric_name, column_name) in enumerate(metrics.items()):
                ...
                ax.set_xticklabels(["Rejected", "Preferred"], fontsize=14)
                ax.tick_params(axis='x', pad=15)
            # Ensure x-axis labels are hidden on all subplots (no-links)
            for ax in fig_no_links.axes:
                ax.set_xlabel("")
                ax.xaxis.label.set_visible(False)
            plt.tight_layout()
            plot_filename_no_links = os.path.join(output_dir, f'combined_reading_measures_{chooser_key}_boxplot.png')
            fig_no_links.savefig(plot_filename_no_links, dpi=300, bbox_inches='tight')
            pdf_filename_no_links = plot_filename_no_links.replace('.png', '.pdf')
            for ax in fig_no_links.axes:
                ax.set_rasterized(True)
            fig_no_links.savefig(pdf_filename_no_links, dpi=300, bbox_inches='tight')
            plt.close(fig_no_links)
            plt.close(fig)
    ...
    def create_boxplots_words_from_paired_data(self, output_dir=None):
        ...
        for metric_name, column_name in metrics.items():
            for chooser_key, chooser_col in chooser_cols:
                ...
                ax_no_links.set_xlabel("", fontsize=14)
    ...
    def create_model_unified_analysis(self, output_dir=None):
        ...
        antonella_data = self.aggregate_data[self.aggregate_data['participant_id'] == 'participant_antonella_antonella'].copy()
        ...
        fig.savefig(pdf_filename, dpi=300, bbox_inches='tight')
        fig_no_links, axes_no_links = plt.subplots(1, 2, figsize=(12, 6))
        fig_no_links.suptitle('Model-Based Preferences', fontsize=18, fontweight='bold')
        for idx, (metric_name, column_name) in enumerate(metrics.items()):
            csv_filename = os.path.join(reading_metrics_dir, f'paired_data_{metric_name}_chosen_model.csv')
            if not os.path.exists(csv_filename):
                continue
...
def main():
    ...
    if aggregate_data is not None:
        ...
        # Apply FDR correction separately for user vs model (4 + 4 tests)
        results_user = {k: v for k, v in results.items() if k.endswith('_user')}
        results_model = {k: v for k, v in results.items() if k.endswith('_model')}
        results_user_fdr = analyzer.apply_fdr_correction(results_user, alpha=0.05)
        results_model_fdr = analyzer.apply_fdr_correction(results_model, alpha=0.05)
        # Merge back into a single dict
        results_fdr = results.copy()
        for sub in (results_user_fdr, results_model_fdr):
        ...
        analyzer.save_summary_statistics(None, results_fdr, words_results_fdr, antonella_model_results=model_results_fdr)
